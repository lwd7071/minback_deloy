...
export async function findStudentByMssvAndClass(
  ...
  if (error) {
    throw new RepositoryError(
      "STUDENT_LOOKUP_FAILED",
      "Không thể tìm kiếm sinh viên theo MSSV",
      { cause: error },
    );
  }
...
import {
  ...
  findClassSectionCodeById,
} from "@/server/repositories/classes/class-section-repository";

/**
 * @deprecated Đã chuyển về class-section-repository.ts theo ranh giới Domain (DDD).
 * Re-export để đảm bảo tương thích ngược cho các callers hiện có.
 */
export { findClassSectionIdByCode, findClassSectionCodeById };
...
export async function updateStudentEmail(
  ...
  if (error) {
    throw new RepositoryError(
      ...
      "Không thể cập nhật email sinh viên",
      { cause: error },
    );
  }
}

export async function findStudentByIdForSession(
  studentId: string,
): Promise<StudentAdminDto | null> {
...
export async function incrementStudentFailedLogin(
  ...
  if (selectError) {
    throw new RepositoryError(
      "STUDENT_FAILED_LOGIN_INCREMENT_FAILED",
      "Không thể lấy số lần đăng nhập thất bại",
      { cause: selectError },
    );
  }
  ...
  if (updateError) {
    throw new RepositoryError(
      "STUDENT_FAILED_LOGIN_INCREMENT_FAILED",
      "Không thể tăng số lần đăng nhập thất bại",
      { cause: updateError },
    );
  }
}
...
export async function resetStudentFailedLogin(
  ...
  if (error) {
    throw new RepositoryError(
      "STUDENT_FAILED_LOGIN_RESET_FAILED",
      "Không thể đặt lại số lần đăng nhập thất bại",
      { cause: error },
    );
  }
}
...
export async function updateStudentPinHash(
  ...
  if (error) {
    throw new RepositoryError(
      "STUDENT_PIN_UPDATE_FAILED",
      "Không thể cập nhật mã PIN",
      { cause: error },
    );
  }
}
...
export async function updateStudentNickname(
  ...
  if (error) {
    ...
    throw new RepositoryError(
      "STUDENT_NICKNAME_UPDATE_FAILED",
