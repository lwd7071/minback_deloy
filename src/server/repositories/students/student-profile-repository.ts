...
import { listActiveAttachmentsForAssignments } from "@/server/repositories/assignments/file-asset-repository";
import { listStudentSubmissionSummaries } from "@/server/repositories/assignments/submission-repository";
...
export async function findStudentProfileData(
  ...
  for (const attachment of attachments) {
    ...
    current.push({
      ...
      downloadUrl: buildStudentAttachmentDownloadUrl(
        ...
        latestAttempt: summary.latestAttempt
          ? {
              ...
              files: summary.latestAttempt.files.map((file) => ({
                ...
                downloadUrl: buildStudentSubmissionFileDownloadUrl(
                  summary.assignmentId,
                  file.id,
                ),
              })),
