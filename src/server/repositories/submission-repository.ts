...
export async function findStudentAssignmentContext(
  ...
  const { data, error } = await createAdminClient()
    ...
    .maybeSingle();
  if (error) throw new Error("SUBMISSION_ASSIGNMENT_LOOKUP_FAILED");
...
export async function getSubmissionAttemptCount(
  ...
  const { data, error } = await createAdminClient()
    ...
    .maybeSingle();
  if (error) throw new Error("SUBMISSION_COUNT_FAILED");
...
async function loadSubmissionHistory(
  ...
  for (const file of files ?? []) {
    ...
    current.push({
      ...
      downloadUrl: `${downloadBaseUrl}/${file.id}/download`,
    });
    filesByAttempt.set(attemptId, current);
  }
...
export async function getStudentSubmissionHistory(
  ...
  const { data, error } = await createAdminClient()
    ...
    .eq("assignment_id", assignmentId)
    .maybeSingle();
  if (error) throw new Error("SUBMISSION_LIST_FAILED");
  if (!data) return { attemptCount: 0, latestAttempt: null, attempts: [] };
  return loadSubmissionHistory(
    data.id as string,
    `/api/v1/student/assignments/${assignmentId}/submission-files`,
  );
}
...
export async function getTeacherStudentSubmissionHistory(
  ...
  return loadSubmissionHistory(
    data.id as string,
    `/api/v1/teacher/assignments/${assignmentId}/submission-files`,
  );
}
...
export async function findSubmissionFileForAssignment(
  ...
  const { data, error } = await createAdminClient()
    ...
    .maybeSingle();
  if (error) throw new Error("SUBMISSION_FILE_LOOKUP_FAILED");
  return data
    ? {
