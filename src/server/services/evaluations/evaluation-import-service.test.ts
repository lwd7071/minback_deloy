...
import { MAX_EVALUATION_IMPORT_FILE_BYTES, normalizeHeaderKey, parseEvaluationCsv, resolveEvaluationColumns } from "./evaluation-import-service";
...
describe("Evaluation Import Service", () => {
  ...
  it("parses quoted CSV records", () => {
    const rows = parseEvaluationCsv(["MSSV,Họ Tên,Điểm,Nhận xét", "22110001,Nguyễn Văn An,9.5,\"Tốt, tiếp tục\""].join("\n"));
  ...
  it("requires four semantic columns and rejects duplicates", () => {
    expect(resolveEvaluationColumns(["MSSV", "Họ tên", "Điểm", "Feedback"])).toEqual({ mssvIndex: 0, nameIndex: 1, scoreIndex: 2, feedbackIndex: 3 });
