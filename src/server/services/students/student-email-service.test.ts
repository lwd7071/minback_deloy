...
vi.mock("@/server/services/notifications/email/safe-email-delivery", () => ({
  deliverEmailSafely: vi.fn(async (send: () => Promise<unknown>) => {
    await send();
    return true;
  }),
}));
...
describe("student email change", () => {
  beforeEach(() => {
    ...
    vi.mocked(findStudentByIdForSession).mockResolvedValue({
      ...
      mssv: "SV01",
      fullName: "Nguyen An",
      email: "sv01@student.hcmute.edu.vn",
      nickname: "SV01",
  ...
  it("updates email only after the OTP is confirmed", async () => {
    ...
    const sentOtp = vi.mocked(
      (
        await import("@/server/services/notifications/email/brevo-email-service")
      ).sendForgotPinOtpEmail,
