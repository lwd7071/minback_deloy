...
test.describe("Kiểm thử: Tạo lớp trống -> Bổ sung sinh viên bằng 2 nút Thêm và Import", () => {
  test("Tạo lớp không kèm sinh viên -> Thêm thủ công & Import file bổ sung thành công", async ({
    ...
    const importBtn = page.locator('button:has-text("Nhập file Excel/CSV")').first();
    ...
    const addModal = page.locator('.modal, [role="dialog"]').filter({ hasText: "Thêm sinh viên vào lớp" });
    ...
    const submitAddBtn = addModal.locator('button[type="submit"]:has-text("Thêm sinh viên")');
    await submitAddBtn.click();

    // Chờ danh sách cập nhật và kiểm tra sinh viên vừa thêm xuất hiện trong bảng
    await expect(page.locator("text=99990001").first()).toBeVisible({ timeout: 10_000 });
    ...
    const importModal = page.locator('.modal, [role="dialog"]').filter({ hasText: "Nhập danh sách sinh viên từ file" });
    ...
    await importModal.locator('input[type="file"]').setInputFiles(studentCsvPath);

    const submitImportBtn = importModal.locator('button[type="submit"]:has-text("Tiến hành nhập")');
    await submitImportBtn.click();

    // Chờ import xong và xác nhận sinh viên từ file xuất hiện trong bảng
    await expect(page.locator("text=21110001").first()).toBeVisible({ timeout: 15_000 });
