...
test.describe("Workflow 3: Re-import Grade, Draft Isolation & Student Realtime Sync", () => {
  test("Giáo viên sửa điểm, kiểm tra chặn lộ bản nháp và học sinh xem điểm cập nhật", async ({
    ...
    await expect(teacherPage).toHaveURL(/\/admin\/classes/, { timeout: 20_000 });
    await teacherPage.waitForLoadState("networkidle");
    ...
    const codeInput = teacherPage.locator('label:has-text("Mã lớp") input, input.form-input').first();
    await codeInput.fill(classCode);
    const nameInput = teacherPage.locator('label:has-text("Tên lớp") input').first();
    ...
    const studentCsvPath = path.resolve(process.cwd(), "test/e2e/danh_sach_sinh_vien_mau.csv");
    ...
    await expect(teacherPage.locator("text=Chuẩn bị danh sách sinh viên")).toBeVisible();
    ...
    await expect(teacherPage.locator(".class-create-stats strong").first()).toHaveText("10", {
      timeout: 30_000,
    });
    await teacherPage.click('button:has-text("Tiếp tục xác nhận")');

    // Bước 3: Xác nhận tạo lớp
    await expect(teacherPage.locator("text=Kiểm tra trước khi tạo")).toBeVisible();
    await teacherPage.locator('button:has-text("Tạo lớp")').last().click();

    // Bước 4: Thành công -> Đi tới lớp
    await expect(teacherPage.locator("h2:has-text('Đã tạo lớp')")).toBeVisible({ timeout: 15_000 });
    ...
    // -------------------------------------------------------------------------
    const assignmentLink = teacherPage.locator('a[href*="/assignments"]').first();
    await assignmentLink.click();
    await teacherPage.waitForLoadState("networkidle");

    await teacherPage.locator('button:has-text("+ Tạo bài tập mới")').click();
    const assignmentTitle = `Bài Tập Phúc Khảo ${randomSuffix}`;
    const titleInput = teacherPage.locator('label:has-text("Tên bài tập") input, input.form-input').first();
    ...
    await expect(teacherPage.locator(`text=${assignmentTitle}`)).toBeVisible({ timeout: 10_000 });
    const gradeLink = teacherPage.locator('a:has-text("Nhập điểm & feedback")').first();
    ...
    // -------------------------------------------------------------------------
    const initialGradePath = path.join(tempDir, `initial_grade_${randomSuffix}.csv`);
    fs.writeFileSync(
      initialGradePath,
    ...
    await expect(modal.locator('.import-preview-table-wrap')).toBeVisible({ timeout: 10_000 });
    ...
    await expect(teacherPage.locator('span:has-text("Đã công bố")')).toBeVisible({ timeout: 10_000 });
    await expect(teacherPage.locator('table.grade-results-table')).toContainText("8");

    // -------------------------------------------------------------------------
    // BƯỚC 4: TEST EDGE CASE - UPLOAD ĐIỂM LỖI (NGOÀI THANG > 10)
    // -------------------------------------------------------------------------
    const invalidGradePath = path.join(tempDir, `invalid_grade_${randomSuffix}.csv`);
    fs.writeFileSync(
      invalidGradePath,
    ...
    await expect(modal.locator('.badge-error').first()).toContainText("vượt quá");

    // Nút Lưu nháp / Công bố phải bị disabled khi không có dòng hợp lệ nào
    await expect(modal.locator('button:has-text("Lưu chưa công bố")')).toBeDisabled();
    await modal.locator('button:has-text("Đóng")').click();

    // -------------------------------------------------------------------------
    // BƯỚC 5: CẬP NHẬT ĐIỂM (9.5) Ở CHẾ ĐỘ "LƯU CHƯA CÔNG BỐ" (LƯU NHÁP)
    // -------------------------------------------------------------------------
    const updatedDraftGradePath = path.join(tempDir, `draft_grade_${randomSuffix}.csv`);
    fs.writeFileSync(
    ...
    await teacherPage.click('button:has-text("Nhập lại file điểm")');
    await modal.locator('input[type="file"]').setInputFiles(updatedDraftGradePath);
    await modal.locator('button:has-text("Kiểm tra tệp")').click();
    ...
    await expect(teacherPage.locator('span:has-text("Chưa công bố")')).toBeVisible({ timeout: 10_000 });
    // Thanh công cụ GV xuất hiện nút "Công bố 1 kết quả"
    await expect(teacherPage.locator('button:has-text("Công bố 1 kết quả")')).toBeVisible();
    ...
    await expect(studentPage).toHaveURL(new RegExp(`/class/${classCode}/onboarding`, "i"), {
      timeout: 10_000,
    });
    await expect(studentPage.locator("text=Để bảo mật tài khoản, bạn cần")).toBeVisible({
      timeout: 10_000,
    });
    ...
    await expect(studentPage).toHaveURL(new RegExp(`/class/${classCode}/(profile|grades)`, "i"), {
      timeout: 15_000,
    });
    await studentPage.waitForLoadState("networkidle");
    ...
    await expect(studentPage.locator("text=Chưa có kết quả được công bố.")).toBeVisible({
      timeout: 10_000,
    });
    ...
    await expect(teacherPage.locator('button:has-text("Công bố 1 kết quả")')).not.toBeVisible({
      timeout: 10_000,
    });
    await expect(teacherPage.locator('table.grade-results-table')).toContainText("9.5");
    ...
    await expect(studentPage.locator(".student-result-score")).toHaveText("9.5 / 10");
    await expect(studentPage.locator(".form-notice")).toContainText("Phúc khảo thành công - bài làm xuất sắc");
